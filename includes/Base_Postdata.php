...
abstract class Base_Postdata extends Base_Data {
	...
	public function read() {
		...
		if ( ! empty( $this->taxonomy_fields ) ) {
			foreach ( $this->taxonomy_fields as $key => $attrs ) {
				if ( $terms = wp_get_object_terms( $post->ID, $attrs['key'], array( 'fields' => 'ids' ) ) ) {
	...
	public function update() {
		...
		if ( ! empty( $post_data ) ) {
			if ( doing_action( 'save_post' ) ) {
				global $wpdb;
				$wpdb->update( $wpdb->posts, $post_data, array( 'ID' => $this->get_id() ) );
			} else {
	...
	public function update_taxonomies( $tax_input = array() ) {
		if ( ! empty( $tax_input ) ) {
